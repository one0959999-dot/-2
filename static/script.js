document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI(data) {
        ...
        // Satellite total calculation removed (moved to fetchKisBalance)
    ...
    // ⚠️ KIS 잔고 조회는 updateUI 안에서 is_mock 확인 후 자동 시작됨
...
window.toggleMode = async function() {
    ...
    try {
        ...
        if (result.status === 'success') {
            // alert 없이 조용히 리로드 (alert가 reload를 막는 문제 방지)
            location.reload();
        } else {
...
window.showStrategyInfo = function(strategyName) {
    ...
    let desc = "";
    if (strategyName.includes("EMA 5/20 크로스")) {
        desc = "최근 5일(단기) 지수이동평균선(EMA)이 20일(장기) 지수이동평균선을 상향 돌파(골든크로스)할 때 매수하고, 하향 돌파(데드크로스)할 때 매도하는 추세 추종 전략입니다.";
    } else if (strategyName.includes("SMA 3/20 크로스")) {
        desc = "3일 단순이동평균선(SMA)과 20일 단순이동평균선의 교차를 활용하여, 단기적으로 빠른 추세 변화를 포착해 진입하는 전략입니다.";
    } else if (strategyName.includes("RSI(14)")) {
        desc = "RSI(상대강도지수)가 30 이하로 떨어지면 과매도 구간으로 판단하여 매수하고, 70 이상으로 올라가면 과매수 구간으로 판단해 매도하는 대표적인 역추세 매매 기법입니다.";
    } else if (strategyName.includes("MACD")) {
        desc = "MACD 선이 Signal 선을 상향 돌파할 때 매수하고, 하향 돌파할 때 매도하여 상승 모멘텀이 시작되는 초입을 노리는 기법입니다.";
    } else if (strategyName.includes("볼린저")) {
        desc = "주가가 볼린저 밴드 하단에 도달했을 때 반등을 예상하여 매수하고, 중심선 또는 상단선에서 매도하는 변동성 돌파 전략입니다.";
    } else {
        desc = "이 전략은 단기적인 모멘텀과 시장 심리를 분석하여 최적의 타점에서 매수/매도를 진행하도록 AI가 13가지 백테스트 후 가장 성과가 좋은 기법으로 자동 선정했습니다.";
    }
...
window.closeStrategyModal = function() {
    ...
    if (strategyAnimReq) {
        cancelAnimationFrame(strategyAnimReq);
        strategyAnimReq = null;
    }
}
...
// 서버에서 기존 코어 종목을 불러와 초기화
(function initCoreList() {
    fetch('/api/status')
        .then(r => r.json())
        .then(data => {
            if (data.cores && data.cores.length > 0) {
                _coreStockList = data.cores.map(c => ({ ticker: c.ticker || '', name: c.name }));
            }
        }).catch(() => {});
})();
...
window.removeCoreStock = function(idx) {
    _coreStockList.splice(idx, 1);
    renderCoreTags();
}

// 종목 검색 (pykrx 종목 목록 - 백엔드 API 사용)
window.searchCoreStock = async function() {
    ...
    try {
        ...
        resultsEl.innerHTML = '<div style="color:#ef4444; font-size:0.85rem; padding:8px;">검색 오류</div>';
    }
}

window.addCoreStock = function(ticker, name) {
    if (_coreStockList.find(s => s.ticker === ticker)) {
        alert('이미 추가된 종목입니다.');
        return;
    }
...
window.saveCoreStocks = async function() {
    ...
    // 기존 API 키 값은 유지한 채 core_stocks만 업데이트
    const data = {
    ...
    try {
        ...
        const result = await res.json();
        if (result.status === 'success') {
            alert('코어 종목이 변경되었습니다. 봇을 재시작합니다.');
            closeCoreModal();
            location.reload();
        } else {
...
window.saveAccountSettings = async function() {
    ...
    try {
        ...
        const result = await res.json();
        if (result.status === 'success') {
            alert('계좌 설정이 저장되었습니다. 시스템 반영을 위해 새로고침합니다.');
            closeSettingsModal();
            location.reload();
        } else {
            alert('저장 실패');
...
window.openReportModal = async function() {
    ...
    try {
        ...
        document.getElementById('report-content').innerHTML = '오류: 리포트를 불러올 수 없습니다.';
    }
}
...
window.checkDailyReport = function() {
    ...
    if (hiddenDate !== today) {
        // 리포트가 생성되었는지 1회 확인 후, 있으면 모달 띄우기
        fetch('/api/daily_report')
            .then(res => res.json())
            .then(json => {
                if (json.status === 'success' && json.data) {
                    // 데이터가 오늘 날짜인 경우에만 띄움
                    if (json.data.date === today) {
                        setTimeout(() => openReportModal(), 1500); // UI 로드 후 약간의 지연
                    }
                }
            })
...
function appendMessage(role, text) {
    ...
    if (role === 'ai') {
        ...
        // 사용자 메시지는 텍스트 그대로 (XSS 방지)
        bubble.textContent = text;
    }

    const time = document.createElement('span');
    time.className = 'chat-msg-time';
    time.textContent = role === 'ai' ? `라씨 AI · ${nowTimeStr()}` : nowTimeStr();
...
window.sendAiMessage = async function() {
    ...
    try {
        ...
        // 채팅이 닫혀있으면 배지 표시
        if (!_aiChatOpen) {
            document.getElementById('ai-new-badge').classList.add('visible');
        }
    } catch (e) {
        hideTyping();
        appendMessage('ai', '⚠️ 서버 통신 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
    } finally {
        _aiIsLoading = false;
    ...
    messages.innerHTML = `
        <div class="chat-msg ai">
            <div class="chat-bubble">
                대화 기록이 초기화되었습니다. 새로운 질문을 해주세요! 🔄
            </div>
            <span class="chat-msg-time">라씨 AI · ${nowTimeStr()}</span>
        </div>`;
}
