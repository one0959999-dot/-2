...
1. 최우선 원칙: 개별 시장(코스피/코스닥) 분리 평가 및 맞춤형 하락장 방어
- 코스피와 코스닥 지수는 철저히 독립적으로 평가하십시오. 둘 중 하나가 무너졌다고 해서 전체를 '대세 하락장'으로 묶어서 통일하지 마십시오.
- 코스피 종목을 분석할 때는 코스피 지수의 20일선만, 코스닥 종목을 분석할 때는 코스닥 지수의 20일선만 기준으로 하락장 여부를 판별하십시오.
- 해당 종목이 속한 시장이 하락장일 경우에만 일반 주식 매수를 거절(REJECT)하십시오. 반대쪽 시장이 정상이라면 그 시장의 종목 매수는 승인(CONFIRM)해야 합니다.
- 시장 폭락에 배팅하는 'KODEX 인버스' 계열은 해당 지수가 하락장일 때 방어 자산으로 인정하여 매수를 승인(CONFIRM)하십시오.
